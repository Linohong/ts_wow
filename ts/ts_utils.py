...
def write_ts_result_to_file(filename, pred_record, only_false_positive_cases=False, true_to_the_top=False):
    l = len(pred_record)
    print_result = []
    if true_to_the_top:
        filename = str(filename) + '_TRUE'
    with open(filename, 'w') as f:
        for original_id, sorted_pred_record in pred_record.items():
            ...
            if true_to_the_top:
                new_pred_record = []
                swap_pos = 0
                random.shuffle(sorted_pred_record) 
                for i, info_dict in enumerate(sorted_pred_record):
                    if str(info_dict['label']).lower() == 'true':
                        swap_pos = i
                        break
                sorted_pred_record[swap_pos], sorted_pred_record[0] = sorted_pred_record[0], sorted_pred_record[swap_pos]
